...
function renderSearchHistoryItems() {
  ...
  for (let i = 0; i < uniqueSearchHistoryItems.length; i++) {
    ...
    searchHistoryBtn.setAttribute(
      "class",
      "bg-white hover:bg-gray-200 text-gray-800 font-semibold py-2 px-4 border border-gray-400 rounded shadow my-1 w-1/3"
    );
  ...
  if (searchHistoryItems.length >= 1) {
    ...
    clearSearchHistoryButton.setAttribute(
      "class",
      "bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-4 rounded my-1"
    );
  }
}
...
async function getRecommendations(type, textInput) {
  ...
  let endpoint;
  if (type === "Artist") {
    endpoint = `https://api.spotify.com/v1/search?q=${encodeURIComponent(
      textInput
    )}&type=artist&limit=1`;
    ...
    endpoint = `https://api.spotify.com/v1/recommendations?limit=1&market=US&seed_genres=${encodeURIComponent(
      textInput
    )}`;
  ...
  for (
    let i = 0;
    i < recommendationsData.tracks.length && recommendationCount < 5;
    i++
  ) {
    ...
    iframe.setAttribute(
      "data-artist",
      recommendationsData.tracks[i].artists[0].name
    );
    ...
    addBtn.setAttribute(
      "class",
      "bg-blue-500 hover:bg-blue-400 text-white font-bold py-2 px-4 border-b-4 border-blue-700 hover:border-blue-500 rounded my-1"
    );
...
function returnElementId123(elementId) {
  ...
  userPlaylistItems.push({
    name: `${clone.getAttribute("data-artist")}`,
    iframe: clone.outerHTML,
  });
  storeUserPlaylist();
...
function renderUserPlaylistItems() {
  ...
  let userPlaylistItems =
    JSON.parse(localStorage.getItem("userPlaylistItems")) || [];
  userPlaylistItems.forEach((item) => {
    ...
    addArtistBtn.setAttribute(
      "class",
      "bg-amber-500 hover:bg-amber-400 text-white font-bold py-2 px-4 border-b-4 border-amber-700 hover:border-amber-500 rounded my-1"
    );
  ...
  if (userPlaylistItems.length >= 1) {
    ...
    clearUserPlaylistButton.setAttribute(
      "class",
      "bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-4 rounded my-1"
    );
  }
...
function loadSavedPlaylist() {
  let storedUserPlaylist = JSON.parse(
    localStorage.getItem("userPlaylistItems")
  );
...
let getArtistId = async function (searchTerm) {
  ...
  let artistEndpoint =
    ...
    searchTerm +
    "&apikey=OG2fkxjdAsu6SUo15migEcOBGuAtVBAL";
...
let getConcertData = async function (artistId) {
  ...
  if (!data2["_embedded"] || !data2._embedded["events"]) {
    ...
    const noConcertsMessage = document.createTextNode(
      "No concerts at this time, please try another artist"
    );
...
let renderConcertData = function (name, date, url) {
  ...
  concertVenue.setAttribute(
    "class",
    "p-2 text-cyan-300 hover:bg-gray-700 hover:text-white focus:outline-none focus:ring-2 focus:ring-inset focus:ring-white"
  );
  concertInfo.setAttribute(
    "class",
    "bg-slate-500 rounded-lg shadow-xl min-h-min p-2 m-2 text-justify"
  );
